...
public class MergeReportService extends AbstractTask {
    ...
    public void execute(RequestContext requestContext, Map<String, Object> map) throws KDException {
        try {
            ...
            Map<String, Map<String, String>> reportAccountsMap = queryAccountsForReports(reportNumbers);

            for (Map.Entry<String, Map<String, String>> entry : reportAccountsMap.entrySet()) {
     ...
     * 为多个报表查询对应的科目
     */
    private Map<String, Map<String, String>> queryAccountsForReports(List<String> reportNumbers) {
        Map<String, Map<String, String>> result = new HashMap<>();

        for (String reportNumber : reportNumbers) {
            Map<String, String> accountMap = queryAccountsForSingleReport(reportNumber);
            if ("GZQY11".equals(reportNumber)) {
                Map<String, String> extraAccountMap = new HashMap<>();
                extraAccountMap.put("SJYS", "税金及附加");
                extraAccountMap.put("XSFY", "销售费用");
                extraAccountMap.put("GLFY", "管理费用");
                extraAccountMap.put("YYWSR", "营业收入");
                extraAccountMap.put("YYWZC", "营业外支出");
                extraAccountMap.put("SJYS14", "税金及附加14");
                for (Map.Entry<String, String> entry : extraAccountMap.entrySet()) {
                    accountMap.putIfAbsent(entry.getKey(), entry.getValue());
                }
            }

            result.put(reportNumber, accountMap);
            logger.info("报表 {} 查询到 {} 个科目", reportNumber, accountMap.size());
        }
     ...
     * 为单个报表查询科目
     */
    private Map<String, String> queryAccountsForSingleReport(String reportNumber) {
...
//        String viewNumber = REPORT_VIEW_MAP.get(reportNumber);
        ...
        DynamicObjectCollection projects = QueryServiceHelper.query("epm_accountmembertree",
                String.join(",", LONGNUMBER_FIELD, SHOWNUMBER, "name"),
                filters);

        Map<String, String> accountMap = new HashMap<>();
        for (DynamicObject obj : projects) {
            String code = obj.getString(SHOWNUMBER);
            String name = obj.getString("name");
            if (code != null && !code.trim().isEmpty()) {
                accountMap.put(code.trim(), name);
            }
        }
